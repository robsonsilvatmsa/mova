<?php
require_once('../../config.php');
require_once("../../vendor/autoload.php");

use Source\models\ProjectAnalyzer;
use Source\controllers\AuditoriaProjeto;
use Source\models\Project;
use Source\models\UserArea;
use Source\models\Group;

$analyzer = new ProjectAnalyzer();
$function = new AuditoriaProjeto();
$area = new UserArea();
abresessao();
$idusu = $_SESSION['idusuario'];


$melhoria = new Project();
$mel = $melhoria->find("status = 5")->fetch(true);
//Nome do pai do menu para identificar
$_SESSION['menu'] = 'Melhoria';

?>


<?php include(ESTILO_TEMPLATE); ?>
<?php include(HEADER_TEMPLATE); ?>
<?php include(MENU_TEMPLATE); ?>

<section role="main" class="content-body">
    <header class="page-header">
        <h2>Auditoria de Projetos</h2>
        <div class="right-wrapper pull-right">
            <ul class="breadcrumbs">
                <li>
                    <a href="#">
                        <i class="fa fa-home"></i>
                    </a>
                </li>
                <li><span>Auditoria de Projetos</span></li>
            </ul>
            <p class="sidebar-right-toggle"></p>
        </div>
    </header>
    <section class="panel">

        <header class="panel-heading">
            <div class="panel-actions">
                <a href="#" class="fa fa-caret-down"></a>
                <a href="#" class="fa fa-times"></a>
            </div>

            <h2 class="panel-title">Auditoria de Projetos</h2>

        </header>
        <div class="panel-body">
            <div class="table-striped">
                <table class="table table-bordered table-striped mb-md" id="datatable-default">
                    <thead>
                    <tr>
                        <th>N° Projeto</th>
                        <th>Descrição</th>
                        <th>Grupo</th>
                        <th>Status</th>
                        <th>Ações</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php if ($mel): ?>
                        <?php foreach ($mel as $value): ?>
                            <tr>
                                <td><?= sprintf('%08d', $value->id); ?></td>
                                <td><?= $value->Descricao; ?></td>
                                <td><?= $group = ((new Group())->findById($value->idGrupo))->name_group ?></td>
                                <td>
                                    <?php
                                    if ($value->Status == 0) {
                                        echo "Enviado para Analise";
                                    } elseif ($value->Status == 1) {
                                        echo "Em Analise";
                                    } elseif ($value->Status == 2) {
                                        echo "Aprovada";
                                    } elseif ($value->Status == 3) {
                                        echo "Implantada";
                                    } elseif ($value->Status == 4) {
                                        echo "Rejeitada";
                                    } elseif ($value->Status == 5) {
                                        echo "Avaliada Equipe Mova";
                                    } elseif ($value->Status == 6) {
                                        echo "Auditado Equipe Mova";
                                    }
                                    ?>
                                </td>
                                <td class="actions">
                                    <a href="<?= BASEURL ?>Projeto/Auditoria/edit.php?id=<?= $value->id ?>"
                                       data-toggle="tooltip"
                                       title="Auditar Projeto">
                                        <i class="fa fa-file-text-o"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <td colspan="10">
                            Não existe dados a listar
                        </td>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
        </div>

    </section>
</section>
<?php include(FOOTER_TEMPLATE); ?>
